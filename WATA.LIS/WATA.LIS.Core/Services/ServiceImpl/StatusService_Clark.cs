...
namespace WATA.LIS.Core.Services.ServiceImpl
{
    public class StatusService_Clark : IStatusService
    {
        ...
        private void MonitoringVisonTimerEvent(object sender, EventArgs e)
        {
            ...
            // Clear QR Code
            if (!m_visionModel.QR.Contains("wata") && m_no_QRcnt > 40 && m_guideSizeComplete == false && m_isPickUp == false)
            {
                m_Command = 0;
                m_event_QRcode = "";
                _eventAggregator.GetEvent<HittingQR_Event>().Publish(m_event_QRcode);
            ...
            if (m_isItemCnt >= 10 && m_isItemCnt % 10 == 0 && m_isPickUp == false)
            {
                ...
                if (Math.Abs(m_event_height - m_curr_height) / 100 > 0.30)
                {
                    if (m_set_item == true)
                    {
