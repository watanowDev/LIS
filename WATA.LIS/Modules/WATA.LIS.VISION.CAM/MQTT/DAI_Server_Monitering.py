    ...
    with dai.Device(fork_pipeline, deviceInfo=fork_device) as device:
        ...
        while True:
            ...
            if inRgb is not None:
                ...
                socket_fork_rgb.send_multipart([buffer.tobytes(), metadata_rgb.encode('utf-8')])
            ...
            if inDepth is not None:
                ...
                socket_fork_depth.send_multipart([buffer.tobytes(), metadata_depth.encode('utf-8')])
            ...
            if inLeft is not None:
                ...
                socket_fork_left.send_multipart([buffer.tobytes(), metadata_left.encode('utf-8')])
            ...
            if inRight is not None:
                ...
                socket_fork_right.send_multipart([buffer.tobytes(), metadata_right.encode('utf-8')])
            ...
            if inVideo is not None:
                ...
                socket_fork_video.send_multipart([buffer.tobytes(), metadata_video.encode('utf-8')])
    ...
    with dai.Device(front_pipeline, deviceInfo=front_device) as device:
        ...
        while True:
            ...
            if inRgb is not None:
                ...
                socket_front_rgb.send_multipart([buffer.tobytes(), metadata.encode('utf-8')])
            if inDet is not None:
                detections = inDet.detections
                detection_data = [{"label": labels[d.label], "confidence": d.confidence,
                                   "xmin": d.xmin, "ymin": d.ymin, "xmax": d.xmax, "ymax": d.ymax,
            ...
            if inDepth is not None:
                ...
                _, buffer = cv2.imencode('.jpg', depthFrame)
                socket_front_depth.send_multipart([buffer.tobytes(), metadata.encode('utf-8')])
            if inLeft is not None:
                leftFrame = inLeft.getCvFrame()
                timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")
                metadata = json.dumps({"timestamp": timestamp})
                _, buffer = cv2.imencode('.jpg', leftFrame)
                socket_front_left.send_multipart([buffer.tobytes(), metadata.encode('utf-8')])
            if inRight is not None:
                rightFrame = inRight.getCvFrame()
                timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")
                metadata = json.dumps({"timestamp": timestamp})
                _, buffer = cv2.imencode('.jpg', rightFrame)
                socket_front_right.send_multipart([buffer.tobytes(), metadata.encode('utf-8')])
            if inVideo is not None:
                videoFrame = inVideo.getCvFrame()
                timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")
                metadata = json.dumps({"timestamp": timestamp})
                _, buffer = cv2.imencode('.jpg', videoFrame)
                socket_front_video.send_multipart([buffer.tobytes(), metadata.encode('utf-8')])
            if cv2.waitKey(1) == ord('q'):
                break
        cv2.destroyAllWindows()
    ...
    with dai.Device(rear_pipeline, deviceInfo=rear_device) as device:
        ...
        while True:
            ...
            if inRgb is not None:
                ...
                metadata = json.dumps({"timestamp": timestamp})
                _, buffer = cv2.imencode('.jpg', frame)
                socket_rear_rgb.send_multipart([buffer.tobytes(), metadata.encode('utf-8')])
            ...
            if inDepth is not None:
                ...
                metadata = json.dumps({"timestamp": timestamp, "roi_depths": roi_depths})
                _, buffer = cv2.imencode('.jpg', depthFrame)
                socket_rear_depth.send_multipart([buffer.tobytes(), metadata.encode('utf-8')])
            ...
            if inLeft is not None:
                ...
                timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")
                metadata = json.dumps({"timestamp": timestamp})
                socket_rear_left.send_multipart([buffer.tobytes(), metadata.encode('utf-8')])
            ...
            if inRight is not None:
                ...
                timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")
                metadata = json.dumps({"timestamp": timestamp})
                socket_rear_right.send_multipart([buffer.tobytes(), metadata.encode('utf-8')])
            ...
            if inVideo is not None:
                ...
                timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")
                metadata = json.dumps({"timestamp": timestamp})
                socket_rear_video.send_multipart([buffer.tobytes(), metadata.encode('utf-8')])
...
# 로딩 라벨: 디바이스가 연결될 때까지 표시
loading_label = tk.Label(root, text="디바이스 연결중...", font=("맑은 고딕", 16))
loading_label.pack(pady=20)

# 디바이스가 연결되면 대시보드(Notebook, Treeview, 상세정보 버튼)를 구성하는 함수
def start_dashboard():
    # 로딩 라벨 제거
    ...
    # 상세 정보 버튼: 각 소켓의 고정 정보(socket_info)와 모니터링 데이터를 함께 출력
