...
# GPU 사용 여부 (CUDA 가속 활성화 여부)
USE_CUDA = True

# ThreadPoolExecutor 생성 (이미지 인코딩 등 병렬처리를 위함)
executor = ThreadPoolExecutor(max_workers=8)

# --- CUDA를 활용한 이미지 인코딩 함수 ---
def encode_image_cuda(frame, quality=5):
    try:
        # GPU로 업로드 (CUDA GpuMat)
        gpu_frame = cv2.cuda_GpuMat()
        gpu_frame.upload(frame)
        # (필요시 GPU 상에서 추가 전처리 수행 가능)
        # GPU에서 다운로드하여 CPU numpy array로 변환
        ...
        print("[ERROR] CUDA encode failed:", e)
        return None

# --- CPU 인코딩 함수 (기존 함수) ---
def encode_image(frame, quality=5):
    encode_param = [int(cv2.IMWRITE_JPEG_QUALITY), quality]
...
# === 5. 각 카메라 파이프라인 실행 함수 (CUDA 및 멀티스레딩 최적화 적용) ===
    ...
    with dai.Device(fork_pipeline, deviceInfo=fork_device) as device:
        ...
        while True:
            ...
            if inRgb is not None:
                ...
                # GPU 인코딩 사용 여부에 따라 선택
                if USE_CUDA:
                    future_rgb = executor.submit(encode_image_cuda, frame, 5)
                else:
            ...
            if inDepth is not None:
                ...
                if USE_CUDA:
                    future_depth = executor.submit(encode_image_cuda, depthFrame)
                else:
            ...
            if inLeft is not None:
                ...
                if USE_CUDA:
                    future_left = executor.submit(encode_image_cuda, leftFrame)
                else:
            ...
            if inRight is not None:
                ...
                if USE_CUDA:
                    future_right = executor.submit(encode_image_cuda, rightFrame)
                else:
            ...
            if inVideo is not None:
                ...
                if USE_CUDA:
                    future_video = executor.submit(encode_image_cuda, videoFrame)
                else:
    ...
    with dai.Device(front_pipeline, deviceInfo=front_device) as device:
        ...
        while True:
            ...
            if inRgb is not None:
                ...
                if USE_CUDA:
                    future_rgb = executor.submit(encode_image_cuda, frame)
                else:
