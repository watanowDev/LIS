...
VISUALIZATION_TIMEOUT = 20.0  # 20초 타임아웃

def check_visualization_timeout():
    """20초 내에 시각화가 시작되지 않으면 프로그램 종료"""
    def timeout_check():
        time.sleep(VISUALIZATION_TIMEOUT)
        if not visualization_started:
            print(f"\nERROR: Visualization window not displayed within {VISUALIZATION_TIMEOUT} seconds!")
...
# Debug output for double-click execution
print(f"Starting StreamingServer...")
print(f"Working directory: {os.getcwd()}")
print(f"Script location: {os.path.dirname(os.path.abspath(__file__))}")
print(f"Visualization timeout: {VISUALIZATION_TIMEOUT} seconds")

# 시각화 타임아웃 체크 시작
check_visualization_timeout()

# 
sys.stdout.reconfigure(line_buffering=True)

# 지정된 fork, front, rear IP 주소로 연결
# fork_info =  dai.DeviceInfo("169.254.1.222")
fork_info =  dai.DeviceInfo("192.168.1.101")
# front_info = dai.DeviceInfo("192.168.2.101")
# rear_info  = dai.DeviceInfo("2.2")
...
print(f"Looking for config file: {configPath.absolute()}")
if not configPath.exists():
    print(f"ERROR: Config file not found at {configPath.absolute()}")
...
# QR 인식 향상을 위한 이미지 전처리 노드 추가
fork_imageManip = fork_pipeline.create(dai.node.ImageManip)
...
# QR 인식 향상을 위한 이미지 전처리 설정
fork_imageManip.initialConfig.setResize(W, H)
fork_imageManip.initialConfig.setFrameType(dai.ImgFrame.Type.BGR888p)
fork_imageManip.setMaxOutputFrameSize(W * H * 3)

# 두 번째 ImageManip 노드는 사용하지 않고 제거
# fork_imageManipSharp는 사용하지 않음
...
# Linking - QR 인식 향상을 위한 파이프라인
# 카메라 → 이미지 전처리 → 객체 감지 네트워크
fork_camRgb.preview.link(fork_imageManip.inputImage)
fork_imageManip.out.link(fork_detectionNetwork.input)
...
def displayNNFrame(name, frame, detections, distances):
    global visualization_started, heartbeat_thread
    ...
    # 창을 맨 앞으로 올리는 코드 제거 - 깜빡거림 방지 및 다른 창이 위에 올라갈 수 있도록 함
    
    # 시각화 화면이 표시되었음을 표시
    if not visualization_started:
        visualization_started = True
        elapsed_time = time.time() - program_start_time
        print(f"Visualization window displayed successfully after {elapsed_time:.2f} seconds")
        
        # 시각화가 시작되면 Heartbeat 쓰레드 시작
        if heartbeat_thread is None:
            heartbeat_thread = threading.Thread(target=send_heartbeat, daemon=True)
            heartbeat_thread.start()
            print("Heartbeat thread started after visualization initialization")
...
def displayDepthFrame(name, depthFrame, detections, distances):
    ...
    for detection in detections:
        if labels[detection.label] == "person":
            ...
            distances.append(int(distance) if distance < 7500 else -1)  # 7.5m 이상이면 -1로 설정


# Define a threading event to control the heartbeat thread
heartbeat_event = threading.Event()
heartbeat_thread = None  # Heartbeat 쓰레드 변수 추가
    ...
    while True:
        ...
        try:
            ...
            if response.status_code == 200:
                print(f"[{datetime.now()}] Heartbeat sent successfully.", flush=True)
            else:
                print(f"[{datetime.now()}] Failed to send heartbeat. Status code: {response.status_code}")
        except Exception as e:
            print(f"[{datetime.now()}] Error sending heartbeat: {e}")
        time.sleep(1)  # 1-second interval for sending heartbeat
...
def process_camera(name, rois, pipeline, device_info, context):
    ...
    with dai.Device(pipeline, devInfo=device_info, usb2Mode=False) as device:
        ...
        while True:
            # 3초 대기 조건
            if time.monotonic() - thread_start_time < 3:
                continue
            ...
            if inDepth is not None:
                ...
                # sockets["depth"].send_multipart([metadata_depth.encode('utf-8')])
            ...
            if inVideo is not None:
                if not heartbeat_event.is_set():
                    heartbeat_event.set()  # Start the heartbeat thread if not already running
                ...
                if heartbeat_event.is_set():
                    heartbeat_event.clear()  # Stop the heartbeat thread if inVideo is None
